...
fn to_pwstr(text: &'static str) -> PWSTR {
...
fn spawn_process(ppid:u64) {
    unsafe {
        ...
        let current_directory = U16CString::from_str("\\??\\C:\\Program Files (x86)\\Microsoft\\Edge\\Application").unwrap();
        ...
        let status = NtOpenProcess(&mut hParent, PROCESS_ALL_ACCESS, &mut oa, &mut cid);
        ...
        if status == 0 || r2 == 0 {
            ...
            println!("Error querying process info: {:x}", status);
        }
...
fn get_process_id_by_name(process_name: &str) -> Result<u32, String> {
    ...
    if h_snapshot == INVALID_HANDLE_VALUE as *mut _ {
...
fn convert_c_array_to_rust_string(buffer: Vec<i8>) -> String {
